...
func addWire(wire string) []line {
	wireParts := strings.Split(wire, ",")
	lines := []line{}
	var x int64 = 0
	var y int64 = 0
	for i := 0; i < len(wireParts); i++ {
		runes := []rune(wireParts[i])
		length, err := strconv.ParseInt(string(runes[1:len(runes)]), 10, 64)
		check(err)
		oldPoint := point{x, y}
		switch runes[0] {
		case 'U':
...
func on(x int64, a int64, b int64) bool {
	if (a <= x && x <= b) || (a >= x && x >= b) {
		return true
	}
	return false
...
func between(x int64, a int64, b int64) bool {
	if (a < x && x < b) || (a > x && x > b) {
		return true
	}
	return false
...
func linesCross(lineA line, lineB line) point {
	if (horizontalLine(lineA) && horizontalLine(lineB)) || (verticalLine(lineA) && verticalLine(lineB)) {
		...
		if between(vLine.a.x, hLine.a.x, hLine.b.x) && between(hLine.a.y, vLine.a.y, vLine.b.y) {
			point := point{vLine.a.x, hLine.a.y}
			//fmt.Println("cross at ",point)
			return point
		}
	}
	return point{0, 0}
...
func pointOnLine(l line, p point) bool {
	if (between(p.x, l.a.x, l.b.x) && (p.y == l.a.y)) || ((p.x == l.a.x) && on(p.y, l.a.y, l.b.y)) {
		return true
	}
	return false
}

func distance(a point, b point) int64 {
	axf := (float64(a.x))
	ayf := (float64(a.y))
	bxf := (float64(b.x))
	byf := (float64(b.y))
	return int64(math.Abs(axf-bxf) + math.Abs(ayf-byf))
}

func distance2point(wire []line, point point) int64 {
	var dist int64 = 0
	i := 0
	for true {
		if pointOnLine(wire[i], point) {
			dist += distance(wire[i].a, point)
			return dist
		}
		dist += distance(wire[i].a, wire[i].b)
		i += 1
	}
	return -1
}
