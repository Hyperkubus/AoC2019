...
func main() {
	...
	if offset < 5000*len(input) {
		log.Fatal("Offset must be in 2nd half")
	}

	fmt.Println("offset:", offset)
	phaseOffset := offset % len(input)
	start := time.Now()
	for i := 0; i < (9999); i++ {
		if offset > len(input) {
			offset -= len(input)
	...
	if phaseOffset != offset {
		log.Fatal("Offset Missmatch")
	}
	...
	fmt.Println(time.Since(start))
	fmt.Println(inputA[offset : offset+8])
}
